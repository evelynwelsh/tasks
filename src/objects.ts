...
export function toShortForm(question: Question): string {
    ...
    const shorterName = newName.substring(0, 10);
    const strID = question.id.toString();
    const combo = strID.concat(": ", shorterName);
    return combo;
}
...
export function toMarkdown(question: Question): string {
    ...
    const beginning = line1 + line2;
    if (question.type === "multiple_choice_question") {
        const newLines = question.options.join("\n- ");
        const start = "\n- ";
        const combine = start + newLines;
        return beginning + combine;
    }
    return beginning;
    return "";
}
